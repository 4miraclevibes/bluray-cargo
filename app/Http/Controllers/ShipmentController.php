...
class ShipmentController extends Controller
{
    public function index()
    {
        if(Auth::user()->role == 'admin'){
            $shipmentOrders = ShipmentOrder::with(['user', 'sender', 'recipient'])->get();
        } else {
            $shipmentOrders = ShipmentOrder::with(['user', 'sender', 'recipient'])->where('user_id', Auth::user()->id)->get();
        }
        return view('pages.backend.shipments.index', compact('shipmentOrders'));
    }
